    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                idx = torch.randperm(xz.size(-1)).to('cuda')
                ...
                if not self.if_devide_out:
                    out = F.linear(rearrange(out + out_b.flip([-1]) + out_r.index_select(dim=-1, index=idx_inverse), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                    # out = F.linear(rearrange(gate_f*(out) + gate_b*(out_b.flip([-1])) + gate_r*(out_r), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                else:
                    # out = F.linear(rearrange(gate_f*(out) + gate_b*(out_b.flip([-1])) + gate_r*(out_r), "b d l -> b l d") / 3, self.out_proj.weight, self.out_proj.bias)
                    out = F.linear(rearrange(out + out_b.flip([-1]) + out_r.index_select(dim=-1, index=idx_inverse), "b d l -> b l d") / 3, self.out_proj.weight, self.out_proj.bias)
            else:
                out = mamba_inner_fn(
                    xz,
                    self.conv1d.weight,
