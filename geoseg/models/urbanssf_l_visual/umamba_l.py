...
class MF(nn.Module):
    ...
    def forward(self, x, res):
        ...
        att = self.attn(res)
        mamba = self.mamba(x, att)
        x = mamba
        x = self.post_conv(x)
        
        return x, att, mamba, res 
...
class Decoder(nn.Module):
    def __init__(self, 
        ...
        encoder_layer = TransformerEncoderLayer(
            ...
            dropout=dropout,
            activation='silu') 
        self.attn = TransformerEncoder(copy.deepcopy(encoder_layer), 6)
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        plt.axis('off')
        plt.imshow(x.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/res4.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        # x = self.attn(x)
        B, C, H, W = x.shape
        ...
        plt.axis('off')
        plt.imshow(x.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/GRE.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        if self.training:
            # 这个地方会增加参数量，res4通道更多
            ah = self.h(x)
        
        x, att, m, res = self.p3(x, res3)
        plt.axis('off')
        plt.imshow(att.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/SRT3.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        plt.axis('off')
        plt.imshow(m.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/mamba3.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        plt.axis('off')
        plt.imshow(res.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/res3.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        x, att, m, res = self.p2(x, res2)
        plt.axis('off')
        plt.imshow(att.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/SRT2.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        plt.axis('off')
        plt.imshow(m.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/mamba2.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        plt.axis('off')
        plt.imshow(res.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/res2.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        x, att, res = self.p1(x, res1)
        plt.axis('off')
        plt.imshow(att.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/SRT1.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        plt.axis('off')
        plt.imshow(x.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/csr.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        plt.axis('off')
        plt.imshow(res.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        plt.savefig('./save/visual/res1.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        plt.close()
        # exit()
...
class UMamba(nn.Module):
    def __init__(self, 
                 decode_channels=256,
                 backbone_name='resnext101_32x4d.fb_swsl_ig1b_ft_in1k',
                 pretrained=True,
                 num_classes=6,
                 depth=6,
                 bimamba_type='v2',
    ...
    def forward(self, x):
        ...
        # import matplotlib.pyplot as plt
        # plt.axis('off')
        # plt.imshow(res1.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        # plt.savefig('./save/visual/res1.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        # plt.close()
        # plt.axis('off')
        # plt.imshow(res2.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        # plt.savefig('./save/visual/res2.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        # plt.close()
        # plt.axis('off')
        # plt.imshow(res3.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        # plt.savefig('./save/visual/res3.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        # plt.close()
        # plt.axis('off')
        # plt.imshow(res4.sum(dim=1).contiguous().data.cpu().numpy()[0, :, :], cmap='RdYlBu')
        # plt.savefig('./save/visual/res4.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        # plt.close()
